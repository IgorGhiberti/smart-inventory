...
- **React 19**
- **Vite 7**
- Fetch API (sem dependências extras)

## 3.2 Estrutura de pastas

- `src/app` - shell, rotas, auth
- `src/features/*` - funcionalidades por domínio
- `src/shared` - UI, api client, utils

## 3.3 Navegação

- Navegação por hash (`#`) com rotas definidas em `src/app/routes.js`.

## 3.4 Autenticação

- Contexto em `src/app/auth-context.jsx`.
- Sessão recuperada via `GET /auth/me`.
- `fetch` sempre com `credentials: 'include'`.

## 3.5 Camada de API

- Client central em `src/shared/api/http-client.js`.
- Tratamento uniforme de erro.
- Redireciona sessão quando recebe `401`.

## 3.6 Telas (features)

- **Produtos**: listagem, baixo estoque, modais de criar/editar, inativar, exibir inativos.
- **Movimentações**: modal de nova movimentação, filtros expansíveis, histórico.
- **Aprovações**: ações por linha (aprovar/rejeitar) com modal.
- **Tipos de movimentação**: modais de criar/editar, inativar.
- **Usuários**: modais de criar/editar, inativar, exibir inativos.

## 3.7 Configuração e deploy

`.env`:

- `VITE_API_BASE_URL` | URL absoluta da API (produção)
- `VITE_PROXY_TARGET` | usado no dev (proxy Vite)
